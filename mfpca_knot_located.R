mfpca_knot_located=function(Y_sparse,time_sparse,group,Q,nknots,nloop,burnin,thin,
	orth=TRUE,sim=FALSE,same=FALSE){
	...
	basis_stuff=basis_sparse_location_spec(time_sparse,nknots,delta=1/600,plotit=TRUE,orth=orth)
	knots=basis_stuff[[1]]
	phi_t=basis_stuff[[3]]
	...
	for(p in 1:P){
		Theta[(sum(K[min(1,p-1):(p-1)])+1):(sum(K[1:p])),sum(Q[min(1,p-1):(p-1)])+1]=mvrnorm(n=1,rep(0,K[p]),diag(rep(1,K[p])))
   	...
   	for(iter in 1:nloop){
		...
		if(est_alpha==1){
			for(i in 1:N){
				...
				Sigma_alpha_i=t(Theta)%*%t(Phi_D_t_i)%*%solve(Sigma_omega_i)%*%Phi_D_t_i%*%Theta+solve(Sigma_alpha)
